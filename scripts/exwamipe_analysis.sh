...
FIXgsi=${FIXgsi:-$HOMEgsi/fix/fix_gsi}
...
CYCLESH=${CYCLESH:-$HOMEwfs/ush/global_cycle.sh}
export CYCLEXEC=${CYCLEXEC:-$HOMEwfs/exec/global_cycle}
...
export FIXgsm=${FIXwsm:-$HOMEwfs/fix/fix_am}

DOGAUSFCANL=${DOGAUSFCANL-"NO"}
GAUSFCANLSH=${GAUSFCANLSH:-$HOMEgfs/ush/gaussian_sfcanl.sh}
export GAUSFCANLEXE=${GAUSFCANLEXE:-$HOMEwfs/exec/gaussian_sfcanl.exe}
...
GPREFIX=${GPREFIX:-""}
GSUFFIX=${GSUFFIX:-""}
SFCG03=${SFCG03:-${COMIN_GES}/${GPREFIX}sfcf03${GSUFFIX}}
SFCG04=${SFCG04:-${COMIN_GES}/${GPREFIX}sfcf04${GSUFFIX}}
SFCG05=${SFCG05:-${COMIN_GES}/${GPREFIX}sfcf05${GSUFFIX}}
SFCGES=${SFCGES:-${COMIN_GES}/${GPREFIX}sfcf06${GSUFFIX}}
SFCG06=${SFCG06:-${COMIN_GES}/${GPREFIX}sfcf06${GSUFFIX}}
SFCG07=${SFCG07:-${COMIN_GES}/${GPREFIX}sfcf07${GSUFFIX}}
SFCG08=${SFCG08:-${COMIN_GES}/${GPREFIX}sfcf08${GSUFFIX}}
SFCG09=${SFCG09:-${COMIN_GES}/${GPREFIX}sfcf09${GSUFFIX}}
ATMG03=${ATMG03:-${COMIN_GES}/${GPREFIX}atmf03${GSUFFIX}}
ATMG04=${ATMG04:-${COMIN_GES}/${GPREFIX}atmf04${GSUFFIX}}
ATMG05=${ATMG05:-${COMIN_GES}/${GPREFIX}atmf05${GSUFFIX}}
ATMGES=${ATMGES:-${COMIN_GES}/${GPREFIX}atmf06${GSUFFIX}}
ATMG07=${ATMG07:-${COMIN_GES}/${GPREFIX}atmf07${GSUFFIX}}
ATMG08=${ATMG08:-${COMIN_GES}/${GPREFIX}atmf08${GSUFFIX}}
ATMG09=${ATMG09:-${COMIN_GES}/${GPREFIX}atmf09${GSUFFIX}}
IPEG03=${IPEG03:-${COMIN_GES}/${GPREFIX}IPE_State.apex.${BDATE}00.h5}
IPEF03=${IPEF03:-${COMIN_GES}/${GPREFIX}IPE_State.apex.${FDATE}00.h5}
IPEGES=${IPEGES:-${COMIN_GES}/${GPREFIX}IPE_State.apex.${CDATE}00.h5}
...
SFCA03=${SFCA03:-${COMOUT}/${APREFIX}sfca03${ASUFFIX}}
...
IPEA03=${IPEANL:-${COMOUT}/${APREFIX}IPE_State.apex.${BDATE}00.h5}
IPEANL=${IPEANL:-${COMOUT}/${APREFIX}IPE_State.apex.${CDATE}00.h5}
IPEB03=${IPEB03:-${COMOUT}/${APREFIX}IPE_State.apex.${FDATE}00.h5}
...
export LONB=${LONB:-$($SIGHDR $ATMGES lonb)}
export LATB=${LATB:-$($SIGHDR $ATMGES latb)}
LEVS=${LEVS:-$($SIGHDR $ATMGES levs)}
export JCAP=${JCAP:-$($SIGHDR $ATMGES jcap)}
...
ANAVINFO=${ANAVINFO:-${FIXgsi}/global_wdas_anavinfo.l${LEVS}.txt}
...
if [ $DOIAU = "YES" ]; then
   export SFCGES=${SFCG03}
   export IPEGES=${IPEG03}
   export SFCANL=${SFCA03}
   export IPEANL=${IPEA03}
fi
if [ $NODA = "YES" ] ; then
   if [ $CDUMP = "wfr" ] ; then
      DOGCYCLE="NO"
      ...
      cp $ATMGES $ATMANL
      cp $SFCGES $SFCANL
      $CHGSIGFHREXEC $ATMANL $FDATE 0.00
      $CHGSFCFHREXEC $SFCANL $FDATE 0.00
      IPEGES=${IPEF03}
      IPEANL=${IPEB03}
   else
      cp $ATMGES $ATMANL
      $CHGSIGFHREXEC $ATMANL $CDATE  0.00
...
if [ $DOGCYCLE = "YES" ]; then
    ...
    if [ $DOIAU = "YES" ]; then
        export SFCGES=${SFCG03}
        export SFCANL=${COMOUT}/${APREFIX}sfca03${ASUFFIX}
    fi
...
TZR_QC=0
DELTIM=60
SETUP="use_gfs_nemsio=.false.,factqmax=0.00,factqmin=0.00,qoption=1,nsig_ext=0,gpstop=30,cwoption=0,$SETUP"
GRIDOPTS="lsidea=.true.,nlayers(63)=1,nlayers(64)=1"
STRONGOPTS="tlnmc_option=0,nstrong=0"
...
if [ $ipe_coldstart = "T" -a $CDUMP = "wdas" ]; then
    ...
    if [ $CDUMP = "wfr" -a $RUN_ENVIR = "emc" ] ; then
        ln -fs $IPEGES $IPEANL
    else
        cp $IPEGES $IPEANL
    fi
